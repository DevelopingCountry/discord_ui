...
} from "@/components/store/use-notification-inbox-store";

export default function NotificationInbox() {
  const { accessToken } = useAuth();
  const router = useRouter();
  const { setServers } = useServerStore();
  const { notifications, unreadCount, setNotifications, removeNotification, markAllRead } = useNotificationInboxStore();
    ...
    if (n.type === "DM") {
      ...
      return (
        <div
          key={n.id}
          onClick={() => {
            router.push(`/channels/me/${payload.dmId}`);
            setOpen(false);
          }}
