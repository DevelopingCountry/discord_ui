...
import { useUpdateServerInfo } from "@/components/hooks/use-update-server-info";
...
export const UpdateServerModal = ({ isOpen, onClose, onBack, currentServerName, serverId }: UpdateServerModalProps) => {
  ...
  // const { mutate,isPending } = useUpdateServerInfo(); pending사용할 때
  ...
  // };
  const handleSubmit = () => {
    console.log("서버 update");
    mutate(
      { serverName: serverName, imageUrl: serverImage, serverId: serverId },
      {
        onSuccess: () => {
          // 기존 서버 목록을 가져오고, 해당 서버 업데이트
          const { servers, setServers } = useServerStore.getState();

          const updatedServers = servers.map((server) =>
            server.id === serverId ? { ...server, name: serverName, imageUrl: serverImage ?? server.imageUrl } : server,
          );
